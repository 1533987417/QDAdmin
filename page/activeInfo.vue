<template>
	<div>
		<v-tab></v-tab>
		<v-search searchName="头条活动查询">
			<template>
				<el-form label-width="100px" label-position="right" :inline="true" :model="serachForm">
					<el-form-item label="活动名称：">
						<el-input v-model="serachForm.ActiveName" placeholder="请输入活动名称" 
						class="input-small"></el-input>
					</el-form-item>
          <el-form-item label="活动类型：">
            <el-input v-model="serachForm.ActiveType" placeholder="请输入活动类型" 
            class="input-small"></el-input>
          </el-form-item>
  ...
  <!-- table组件 -->
  <v-table title="活动列表" :totalRecords="totalCount" ref="table" @pageChange="pageChange">
   <template slot="btn">
    <el-button type="primary" size="small" @click="addDialog">新增</el-button>
  </template>
  <el-table :data="tableData" border  style="width: 100%">
    <el-table-column align="center" type="index" label="序号" width="60"> </el-table-column>
    <el-table-column align="center" prop="ActiveName" label="活动标题" width="200"></el-table-column>
    <el-table-column align="center" prop="ActiveDetail" label="活动详情" min-width="360"></el-table-column>
    <el-table-column align="center" prop="ActiveType" label="活动类型" width="120">

    </el-table-column>
    <el-table-column align="center" prop="Status" label="状态" width="50">
     <template slot-scope="props">{{props.row.Status==1?"启用":"无效"}}</template>
   </el-table-column>
   <el-table-column align="center" prop="ActiveStartDate" label="活动开始时间" width="180">
     <template slot-scope="props">
      {{new Date(props.row.ActiveStartDate).ljyFormat("yyyy-MM-dd HH:mm")}}
    </template>
  </el-table-column>
  <el-table-column align="center" prop="ActiveEndDate" label="活动结束时间" width="180">
   <template slot-scope="props">
    {{new Date(props.row.ActiveEndDate).ljyFormat("yyyy-MM-dd HH:mm")}}
  </template>
</el-table-column>
    ...
    totalCount:0,
     ...
     methods: {
      ...
      removeItem(e){
      	...
      	http.httpPost("/manager/activeInfo/updateActiveInfo",para).then(data=>{
      		console.log(data)
      ...
      save(){
        ...
        if(this.isAdd){
          this.$refs['form'].validate((valid) => {
           ...
           http.httpPost("/manager/activeInfo/addActiveInfo",this.form).then(data=>{
             console.log(data)
      ...
      search(){  // 查询函数
      	...
      	console.log(para);
      	http.httpPost("/manager/activeInfo/getActiveInfoList",para).then(data=>{
